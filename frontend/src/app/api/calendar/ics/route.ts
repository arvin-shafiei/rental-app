...
export async function GET(request: NextRequest) {
  try {
    ...
    // Forward the request to backend - remove duplicate /api
    const response = await fetch(`${API_URL}/calendar/ics`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        return NextResponse.json(
          { success: false, message: 'Calendar not found' },
          { status: 404 }
        );
      }
    ...
    console.error('Error in GET /api/calendar/ics:', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.title || !body.startDateTime || !body.endDateTime) {
      ...
      return NextResponse.json(
        { success: false, message: 'Missing required fields: title, startDateTime, endDateTime' },
        { status: 400 }
      );
    }
    ...
    const response = await fetch(`${API_URL}/calendar/ics`, {
      ...
      console.error('Network error posting to calendar:', error);
      throw new Error('Network error connecting to backend');
    });
    ...
    console.error('Error in POST /api/calendar/ics:', error);
    return NextResponse.json(
