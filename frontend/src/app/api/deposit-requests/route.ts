...
export async function GET(req: NextRequest) {
  try {
    ...
    if (tokenError || !tokenData.user) {
      console.error('Invalid token in deposit-requests route:', tokenError);
      return NextResponse.json({ error: 'Authentication required' }, { status: 401 });
    }
    
    // Forward the request to the backend
    const response = await fetch(`${API_URL}/deposit-requests?propertyId=${propertyId}`, {
      headers: {
    ...
    console.error('Error in deposit-requests API route:', error);
    return NextResponse.json(
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (userError || !userData.user) {
      console.error('Invalid token in deposit-requests route:', userError);
